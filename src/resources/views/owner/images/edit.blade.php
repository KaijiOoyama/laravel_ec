                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('owner.images.update', ['image' => $image->id ]) }}">
                        ...
                        <div class="p-2 w-full mt-3 flex justify-around">
                            <button type="button" onclick="location.href='{{ route('owner.images.index') }}'" class="mx-auto text-white bg-gray-200 border-0 py-2 px-8 focus:outline-none hover:bg-purple-600 rounded text-lg">
                                戻る
                            </button>
                    ...
                    <form id="delete_{{$image->id}}" action="{{ route('owner.images.destroy', ['image' => $image->id ]) }}" method="POST">
                        @method('delete')
                        @csrf
                        <div class="w-24 h-20 text-center">
                            <a data-id="{{$image->id}}" onclick="deletePost(this)" href="#" class="mx-auto text-white bg-red-500 border-0 py-2 px-2 focus:outline-none hover:bg-purple-600 rounded">
                                削除する
                            </a>
                        </div>
                    </form>
